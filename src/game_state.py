...
class GameState:
    def __init__(self, map: Map, large = False):
        self.map = map
    ...
    def render_large(self):
        ...
        for team in [Team.BLUE, Team.RED]:
            for tower in self.towers[team].values():
                ...
                pygame.draw.circle(self.screen, color, center, 30)
                pygame.draw.circle(self.screen, innercolor, center, 20)
        ...
        for team in [Team.BLUE, Team.RED]:
            for x in range(self.map.width):
                for y in range(self.map.height):
                    if counts[team][x][y] == 0:
                        continue
                    text = font.SysFont('Comic Sans MS', 50).render(str(counts[team][x][y]), True, (255, 255, 255))
                    ((left, top), (width, height)) = get_screen_coords(team, x, y)
                    center = (left + width/2, top + height/2)
                    text_rect = text.get_rect(center=center)
                    self.screen.blit(text, text_rect)
        ...
        for team in [Team.BLUE, Team.RED]:
            for (x, y) in self.current_bombs[team]:
                ...
                pygame.draw.circle(
                    ...
                    center,
                    math.sqrt(TowerType.BOMBER.range) * self.tile_size,
                    5 # circle outline width
                )

        # Draw turn number in bottom left
        text = font.SysFont('Comic Sans MS', 50).render(f"Turn: {self.turn}", True, (255, 255, 255))
        self.screen.blit(text, ((2, self.screen.get_height()-40), (self.screen.get_width()//2, 20)))

        # Draw each team's balance
        text = font.SysFont('Comic Sans MS', 50).render(f"Balance: {self.balance[Team.BLUE]}", True, (255, 255, 255))
        self.screen.blit(text, ((2, 0), (self.screen.get_width()//2, 40)))
        text = font.SysFont('Comic Sans MS', 50).render(f"Balance: {self.balance[Team.RED]}", True, (255, 255, 255))
        self.screen.blit(text, ((self.screen.get_width()//2+2, 0), (self.screen.get_width()//2, 40)))

        # Draw each team's health
        text = font.SysFont('Comic Sans MS', 50).render(f"Health: {self.health[Team.BLUE]}", True, (255, 255, 255))
        self.screen.blit(text, ((2, 40), (self.screen.get_width()//2, 80)))
        text = font.SysFont('Comic Sans MS', 50).render(f"Health: {self.health[Team.RED]}", True, (255, 255, 255))
        self.screen.blit(text, ((self.screen.get_width()//2+2, 40), (self.screen.get_width()//2, 80)))

        # Draw each team's time remaining
        text = font.SysFont('Comic Sans MS', 50).render(f"Time: {self.time_remaining[Team.BLUE]: .2f}", True, (255, 255, 255))
        self.screen.blit(text, ((2, 80), (self.screen.get_width()//2, 120)))
        text = font.SysFont('Comic Sans MS', 50).render(f"Time: {self.time_remaining[Team.RED]: .2f}", True, (255, 255, 255))
